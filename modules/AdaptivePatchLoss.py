...
class AdaptivePatchLoss(nn.Module):
    ...
    def forward(self, attention_weights, fixed_pos_embeds, interpolated_pos_embeds):
        cls_to_patches = self.attention_rollout(attention_weights)
        scaled_attention = torch.pow(cls_to_patches, 1.0 / self.temperature)
        scaled_attention /= scaled_attention.sum(dim=-1, keepdim=True) + 1e-9
        attn_loss = self.compute_attention_loss(scaled_attention)
        diversity_loss = self.compute_diversity_loss(fixed_pos_embeds, interpolated_pos_embeds)
    ...
    def attention_rollout(self, attention_weights):
        rollout = attention_weights[0, :, 0, 1:]
        for layer in attention_weights[1:]:
            rollout = torch.bmm(rollout.unsqueeze(1), layer[:, 1:, 1:]).squeeze(1)
            rollout /= rollout.sum(dim=-1, keepdim=True) + 1e-9
        return rollout
